...
| [`v0.4.2-rc.1-candidate-preparation.md`](v0.4.2-rc.1-candidate-preparation.md) | **INVALIDATED.** I3.2's candidate freeze and full automated gate-table qualification for `v0.4.2-rc.1`, `71e2d8b954fa92430723fced302baa3666255397`. I3.3's actual-client qualification against this candidate found a release-blocking single-request denial-of-service in the MCP transport (real Claude Code traffic hung the server indefinitely); see [`v0.4.2-client-traces/claude-code.md`](v0.4.2-client-traces/claude-code.md) and the fix recorded in [ADR 0014's Amendment](../adr/0014-negotiated-mcp-client-interoperability.md). Superseded by `v0.4.2-rc.2`. |
| [`v0.4.2-rc.2-candidate-preparation.md`](v0.4.2-rc.2-candidate-preparation.md) | **INVALIDATED.** I3.2's re-freeze after `rc.1`'s invalidation: full automated gate-table re-qualification for `v0.4.2-rc.2`, `6461db6d51ee29e9c973e62b005aa84d5d95c077`, which includes the direct-marked-method-allowlist transport fix. I3.4's actual-client qualification against this candidate found a second release-blocking transport defect: AIP's negotiated-mode guard rejected any real client's markerless follow-up missing an `MCP-Protocol-Version` header, stricter than the pinned SDK it wraps and than the MCP spec's own server-side backward-compatibility allowance for that exact (client-nonconformant but real) traffic — VS Code/GitHub Copilot Chat could not connect at all; see [`v0.4.2-client-traces/vscode.md`](v0.4.2-client-traces/vscode.md) and the fix recorded in [ADR 0014's second Amendment](../adr/0014-negotiated-mcp-client-interoperability.md). Requires a new candidate; to be superseded once one is frozen. |
